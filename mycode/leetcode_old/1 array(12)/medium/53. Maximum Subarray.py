# coding:utf-8
# 2级,3级
# 题目:Find the contiguous subarray within an array (containing at least one number) which has the largest sum.求和最大的连续子数组
#     例如list = [-2,1,-3,4,-1,2,1,-5,4],the contiguous subarray [4,−1,2,1] has the largest sum = 6.
# 思路: 我的思路就是两个指针.但是超时
#       他们的思路:我们用local[i]来表示以A[i]结尾的连续子数组的最大和，那么local[0] = A[0],,  local[1]等于要么local[0]+A[1]  要么就等于A[1]本身，
#       关键看两者谁更大，于是便得到递归式local[i+1] = max(local[i]+A[i], A[i]),,,,local数组中的最大值即为所求。
#       迭代的过程中无需数组 因此时间复杂度为O(N), 空间复杂度为O(1).

def maxSubArray(A):
    current = 0
    result = A[0]
    for i in A:
        current += i  #current在累积,但是如果是负数,累积那段就不要了,如果是正数,就跟之前的比较.留下较大的
